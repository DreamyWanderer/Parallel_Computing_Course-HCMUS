					 ...
					 bool useDevice = false, dim3 blockSize = dim3(1))
{
	...
	else // use device
	{
		...
		uint8_t *d_inPixel, *d_outPixel;
		const int sizeOutPixel = height * width * sizeof(uint8_t);
		const int sizeInPixel = height * width * 3 * sizeof(uint8_t);
		CHECK(cudaMalloc(&d_inPixel, sizeInPixel));
		CHECK(cudaMalloc(&d_outPixel, sizeOutPixel));

		// TODO: Copy data to device memories
		CHECK(cudaMemcpy(d_inPixel, inPixels, sizeInPixel, cudaMemcpyHostToDevice));
		CHECK(cudaMemcpy(d_outPixel, outPixels, sizeOutPixel, cudaMemcpyHostToDevice));
		...
		convertRgb2GrayKernel<<<gridSize, blockSize>>>(d_inPixel, width, height, d_outPixel);
		cudaDeviceSynchronize();
		CHECK(cudaGetLastError());

		// TODO: Copy result from device memories
		CHECK(cudaMemcpy(outPixels, d_outPixel, sizeOutPixel, cudaMemcpyDeviceToHost));
